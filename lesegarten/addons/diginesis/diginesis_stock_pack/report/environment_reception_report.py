...
class EnvironmentReceptionReport(models.AbstractModel):
    _name = 'environment.reception.report'
    _description = 'Environment Reception Report'
    ...
    def _get_templates(self):
        templates = super(EnvironmentReceptionReport, self)._get_templates()
    ...
    @api.model
    def _create_environment_reception_report_line(self, options, vals):
        prec_digits = 1
    ...
    FROM (
        SELECT
            ...
            COALESCE(stock_picking.name,'') || ' - ' ||min(COALESCE(account_notice.name, account_move.name, '')) AS document_number, 
            min(COALESCE(date(stock_picking.date), account_notice.date, account_move.invoice_date)) AS document_date, 
            stock_tracking.date AS tracking_date, 
        ...
        FROM
            ...
            JOIN stock_location ON stock_location.id = stock_move.location_id
            ...
            LEFT JOIN account_move ON account_move.id=stock_picking.vendorbill_id
            LEFT JOIN account_notice ON account_notice.id=stock_picking.notice_id               
        ...
        WHERE
            stock_move.state='done' AND stock_location.usage='supplier'
    ...
    ORDER BY b.document_date
        ...
        params = {
            'foil_coefficient': company.stock_packaging_foil_coefficient or 0.2,
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for vals in self._fill_missing_values(query_res):
            line = self._create_environment_reception_report_line(options, vals)
            if line:
                lines.append(line)
    ...
    def _get_report_name(self):
        return _('Environment Reception Report')

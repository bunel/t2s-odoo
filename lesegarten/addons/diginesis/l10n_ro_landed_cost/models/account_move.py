...
class AccountMove(models.Model):
    ...
    landed_costs_ids = fields.Many2many('l10n.ro.stock.landed.cost', 'landed_cost_vendor_bill_rel', 'vendor_bill_id', 'landed_cost_id', string='Landed Costs')
    ...
    def button_create_landed_costs(self):
        """Create a `l10n.ro.stock.landed.cost` record associated to the account move of `self`, each
        `l10n.ro.stock.landed.cost` lines mirroring the current `account.move.line` of self.
        """
        self.ensure_one()
        landed_costs_lines = self.line_ids.filtered(lambda line: line.is_landed_costs_line)

        landed_costs = self.env['l10n.ro.stock.landed.cost'].create({
            'vendor_bill_ids': [(6, 0, self.ids)],
            'cost_lines': [(0, 0, {
                'product_id': l.product_id.id,
                'name': l.product_id.name,
                'account_id': l.account_id.id,
                'price_unit': l.balance,
                'split_method': l.product_id.split_method_landed_cost or 'by_value',
            }) for l in landed_costs_lines],
        })
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_ro_landed_cost.action_l10n_ro_stock_landed_cost")
        return dict(action, view_mode='form', res_id=landed_costs.id, views=[(False, 'form')])

    def action_view_landed_costs(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_ro_landed_cost.action_l10n_ro_stock_landed_cost")
        action['domain'] = [('id', 'in', self.landed_costs_ids.ids)]
        action['context'] = dict(self.env.context, default_vendor_bill_id=self.id)
        return action
